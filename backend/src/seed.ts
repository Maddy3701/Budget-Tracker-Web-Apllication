...
import { User, Transaction, Goal } from './models';
...
const seed = async () => {
  ...
  await User.deleteMany({ email: 'demo@budget.app' });
  ...
  for (let m = 0; m < 3; m++) {
    ...
    // Income
    transactions.push(
      { userId: uid, type: 'income', category: 'Salary', amount: 50000, date: new Date(year, month, 1), note: 'Monthly salary' },
      { userId: uid, type: 'income', category: 'Pocket Money', amount: 5000, date: new Date(year, month, 5), note: 'Extra pocket money' },
    );
